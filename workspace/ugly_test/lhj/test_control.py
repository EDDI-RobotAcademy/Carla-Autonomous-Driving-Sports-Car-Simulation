...
def calculate_control(vehicle, target_velocity):
    # Get current vehicle velocity
    vehicle_velocity = vehicle.get_velocity()
    # Calculate throttle command
    throttle = min(target_velocity, 5)  # Assuming max speed of 5 m/s
            ...
            if event.type == pygame.QUIT:
                ...
                if isinstance(self._control, carla.VehicleControl):
                    if event.key == K_q:
                        ...
                        world.player.set_autopilot(not world.player.is_autopilot_enabled())
                        if world.player.is_autopilot_enabled():
                            # Set destination for autopilot to target_location
                            target_location = carla.Location(x=24.3, y=-25,
                                                             z=2)  # Modify to create a carla.Location object
                            blueprint = world.get_blueprint_library().find('sensor.other.gnss')
                            spawn_point = carla.Transform(carla.Location(x=0, y=-84.5, z=2))
                            gnss = world.spawn_actor(blueprint, spawn_point)
                            world.tick()  # Wait for the actor to start sensing
                            gnss_data = gnss.listen(lambda data: None)
                            world.player.set_autopilot(True)
                            world.player.set_destination(gnss_data, target_location, clean=True)
                            print("Autopilot enabled. Heading to target location.")
                            # Continuously check if the vehicle reached the target waypoint
                            while True:
                                if check_reached_target(world.player.get_transform(),
                                                        target_location):  # Pass vehicle transform instead of location
                                    control = carla.VehicleControl()
                                    control.throttle = 0  # Stop the vehicle
                                    world.player.apply_control(control)
                                    print("Vehicle reached the target waypoint. Autopilot mode deactivated.")
                                    world.player.set_autopilot(False)
                                    break  # Exit the loop if the vehicle reaches the target waypoint
                                time.sleep(0.1)  # Add a short delay to avoid excessive checking
                        else:
...
def game_loop(args):
    ...
    target_location = carla.Transform(carla.Location(x=24.3, y=-25, z=2), carla.Rotation())
    pygame.init()
    pygame.font.init()
    world = None
    ...
    try:
        ...
        while True:
            ...
            control = calculate_control(world.player, target_velocity)

            # Apply control command
            world.player.apply_control(control)

            # Check if the vehicle reached the target waypoint
            if check_reached_target(world.player, target_location):
                control.throttle = 0  # Stop the vehicle
                world.player.apply_control(control)
                break  # Exit the loop if the vehicle reaches the target waypoint
