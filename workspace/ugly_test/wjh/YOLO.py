...
def run(
        ...
        source=ROOT / display,  # file/dir/URL/glob/screen/0(webcam) 'data/images'
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            txt_path = '{}{}'.format(str(save_dir / 'labels' / p.stem), '' if dataset.mode == 'image' else '_{}'.format(frame))
  # im.txt
            ...
            if len(det):
                ...
                for detection in reversed(det):
                    xyxy = detection[:4]
                    conf = detection[4]
                    cls = int(detection[5])  # integer class
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        # label format
                        if save_conf:
                            line = (cls,) + tuple(xywh) + (conf,)
                        else:
                            line = (cls,) + tuple(xywh)
                    ...
                    if save_crop:
                        save_one_box(xyxy, imc, file=os.path.join(save_dir, 'crops', names[cls], '%s.jpg' % p.stem), BGR=True)
        ...
        if len(det):
            LOGGER.info("{}{:.1f}ms".format(s, dt[1].dt * 1E3))
        else:
            LOGGER.info("{}(no detections), {:.1f}ms".format(s, dt[1].dt * 1E3))

    # Print results
    t = tuple(x.t / seen * 1E3 for x in dt)  # speeds per image
    LOGGER.info('Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape %s' % (t[0], t[1], t[2], (1, 3) + imgsz))

    if save_txt or save_img:
        if save_txt:
            s = "\n{} labels saved to {}".format(len(list(save_dir.glob('labels/*.txt'))), save_dir / 'labels')
        else:
            s = ''
